...
def read_backup_header(backup_file: BinaryIO) -> HeaderData:
    ...
    # print("header len: {}".format(length))
    ...
    if backup_frame.header.HasField("version"):
        ...
        print("header version: {}".format(header_version))
    ...
    print("iv: {}".format(headerData.initialisation_vector))
    print("salt: {}".format(headerData.salt))
    # print("header version: {}".format(headerData.version))
...
def decrypt_frame(
    ...
    if header_version is None:
        ...
        hmac.update(encrypted_length) # encrypted length bytes
    ...
    assert length >= 10
    print("Reading {} bytes frame".format(length-10))
...
def decrypt_frame_payload(
    ...
    # print("iv: {}".format(initialisation_vector))
    ...
    print("Reading {} bytes payload".format(length))
    while length > 0:
...
def decrypt_backup(
    ...
    db_backup = open(output_directory._str + "/db_backup.txt", "w")
    ...
    while True:
        ...
        if backup_frame.HasField("end"):
            ...
            db_backup.write(f"PRAGMA user_version = {backup_frame.version.version:d}\n")
            db_cursor.execute(
            ...
            if (
                ...
                db_backup.write(f"{statement.statement} ({statement.parameters})\n")
                db_cursor.execute(
                    statement.statement,
